...
В ходе работы было спроектирвано и отвергнуто множество вариантов интерфейса сайта. Работа над внешней составляющей и удобством использования продолжалась в течение всего срока разработки форума. HTML5 дал много возможностей для оптимальной разметки сайта, причем такой, что все хорошо ресайзится под любой размер экрана девайса.
...
Для получения обратной связи клиентом посланный им запрос идентифицируется с помощью паттернов в файле urls.py. Далее необходимые аругменты передаются на вход некоторому "вью" (функции из view.py), которая, в свою очередь, выполняя определенные действия, возвращает, если требуется, новую страницу.
